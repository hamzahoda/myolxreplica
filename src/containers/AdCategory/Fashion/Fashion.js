...
import fashionadclasses from '../../Mainads/Mainads.module.css'
...
class Fashion extends Component{
    ...
    componentDidMount(){
        ...
        let newads=[]
    //     firebase.database().ref("/").child("ads").orderByChild("subcategory").equalTo("fashion").on("child_added",
    // (data)=>{
    //     newads.push(data.val())
    //     this.setState({ads:newads,showspinner:false})}
    // )
    firebase.database().ref('/').child("ads").orderByChild("subcategory").equalTo("Fashion").once('value',  (snapshot) =>{
    ...
    render(){
            ...
            return (
                <Aux>
                <img src={mainimageresize} className={fashionadclasses.mainheaderimage} alt="Mainheader"/>
                <h1 className={fashionadclasses.admainheading}>Fashion</h1>
                {this.state.showspinner?<Spinner />:
                <div>
                <div className={fashionadclasses.adcontainer}>
                {
                adnumber.map((ad,i) => {
                return <Singlead
                key={ad.key} 
                ...
                {this.state.loadmorebtn?<button onClick={this.loadMoreHandler} className={fashionadclasses.Loadmore}>Load more</button>:
                this.state.errormessage?
                <p className={fashionadclasses.Errormessage}>Error Occured Please Refresh the page</p>
                :
                <p className={fashionadclasses.Endresult}>End of Result</p>
                }
                </div>}
                </Aux>
                )
