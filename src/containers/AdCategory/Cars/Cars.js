...
import caradclasses from '../../Mainads/Mainads.module.css'
...
class Car extends Component{
    ...
    // )
    firebase.database().ref('/').child("ads").orderByChild("subcategory").equalTo("Cars").once('value',  (snapshot) =>{
    ...
    render(){
            ...
            return (
                <Aux>
                <img src={mainimageresize} className={caradclasses.mainheaderimage} alt="Mainheader"/>
                <h1 className={caradclasses.admainheading}>Car</h1>
                {this.state.showspinner?<Spinner />:
                <div>
                <div className={caradclasses.adcontainer}>
                {
                adnumber.map((ad,i) => {
                return <Singlead
                key={ad.key} 
                ...
                {this.state.loadmorebtn?<button onClick={this.loadMoreHandler} className={caradclasses.Loadmore}>Load more</button>:
                this.state.errormessage?
                <p className={caradclasses.Errormessage}>Error Occured Please Refresh the page</p>
                :
                <p className={caradclasses.Endresult}>End of Result</p>
                }
                </div>}
                </Aux>
                )
